...
function toggleSimuladoCardExpansion(clickedCard, categoryId) {
    ...
    const isExpandingThisCard = !clickedCard.classList.contains('expanded');
    ...
    // Limpa o conteúdo do detailsDiv antes de qualquer coisa para evitar acúmulo
    const detailsDivClicked = clickedCard.querySelector('.simulado-card-details');
    if (detailsDivClicked) detailsDivClicked.innerHTML = '';

    // Se estiver expandindo, repopula. Se estiver colapsando, a limpeza já foi feita.
    if (isExpandingThisCard) {
        const simuladoId = clickedCard.dataset.simuladoId;
        const simulado = simuladosState.simulados.find(s => s.id === simuladoId);
        if (simulado && detailsDivClicked) {
            const percentualAcertos = simulado.totalQuestoes > 0 ? Math.round((simulado.acertos / simulado.totalQuestoes) * 100) : 0;
            const desempenhoTexto = getDesempenhoTexto(percentualAcertos);
            const desempenhoClasse = getDesempenhoClasse(percentualAcertos);
            detailsDivClicked.innerHTML = `
                <div class="simulado-details-grid">
                    <div class="simulado-metric-display">
                        <div class="metric-header"><i class="bi bi-check2-circle metric-icon"></i><span class="metric-label">Acertos</span></div>
                        <div class="metric-value">${simulado.acertos} <span class="metric-value-small">/ ${simulado.totalQuestoes} (${percentualAcertos}%)</span></div>
                    </div>
                    <div class="simulado-metric-display">
                        <div class="metric-header"><i class="bi bi-clock-history metric-icon"></i><span class="metric-label">Tempo</span></div>
                        <div class="metric-value">${simulado.tempoGastoMinutos ? formatMinutesToHHMM(simulado.tempoGastoMinutos) : '--'}</div>
                    </div>
                </div>
                <div class="simulado-progress-section">
                    <div class="simulado-progress-header"><span class="progress-label-text">Percentual de Acertos</span><span class="progress-percentage-text">${percentualAcertos}%</span></div>
                    <div class="simulado-progress-bar-container"><div class="simulado-progress-bar"><div class="simulado-progress-fill" style="width: ${percentualAcertos}%;"></div></div></div>
                </div>
                <div class="simulado-performance-section"><span class="performance-label"><i class="bi bi-award-fill"></i>Desempenho</span><span class="performance-badge ${desempenhoClasse}">${desempenhoTexto}</span></div>
                ${simulado.status ? `<div class="simulado-card-status-info">Status: <strong>${formatSimuladoStatus(simulado.status)}</strong></div>` : ''}
            `;
        }
    }
    
    simuladosState.expandedRowTracker[categoryId][rowIndexOfClickedCard] = isExpandingThisCard;
                ...
                if (card.classList.contains('expanded')) {
                    ...
                    setTimeout(() => {
                        if (!card.classList.contains('expanded')) {
                            ...
                            detailsDiv.innerHTML = ''; // Limpa ao fechar para evitar acúmulo
                        }
                    }, 350); 
                }
            }
